...
const WeekCalendarEvent = ({
  ...
  campaign,
  event,
}: WeekCalendarEventProps): JSX.Element => {
  ...
  startOfDay = new Date(
    ...
    startOfDay.getUTCHours(),
    startOfDay.getUTCMinutes()
  );
  ...
  const startTime = new Date(
    naiveStartDate.getUTCFullYear(),
    naiveStartDate.getUTCMonth(),
    naiveStartDate.getUTCDate(),
    naiveStartDate.getUTCHours(),
    naiveStartDate.getUTCMinutes()
  );

  const endTime = new Date(
    naiveEndDate.getUTCFullYear(),
    naiveEndDate.getUTCMonth(),
    naiveEndDate.getUTCDate(),
    naiveEndDate.getUTCHours(),
    naiveEndDate.getUTCMinutes()
  );
  ...
  const getEventPos = () => {
    ...
    const startFromMidnight = startsBeforeToday
      ? 0
      : (startTime.getTime() - new Date(startTime).setHours(0, 0, 0, 0)) /
        60000;

    const endFromMidnight = endsAfterToday
      ? 24 * 60
      : (endTime.getTime() - new Date(endTime).setHours(0, 0, 0, 0)) / 60000;
  ...
  const getEventTimeLabel = () => {
    return (
      <>
        <FormattedTime
          value={new Date(startsBeforeToday ? startOfDay : startTime)}
        />
        {` - `}
      </>
    );
            ...
            >
              {maxNoOfLabels > 0 && (
                <Typography ref={label} noWrap variant="body2">
                  <span data-testid={`start-time-${event.id}`}>
                    {getEventTimeLabel()}
                  </span>
